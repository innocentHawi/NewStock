...
<body class="bg-blue-100">
    <div class="flex justify-center items-center h-screen">
        <div class="bg-white border-2 border-black rounded-lg py-6 px-8">
            ...
            <form action="/edit-stock/{{$stock->id}}" method="POST">
                @csrf
                @method('PUT')
                <label for="stock_name" class="mb-2 text-gray-700">Stock Name</label>
                <input type="text" name="stock_name" value="{{$stock->stock_name}}" disabled
                    class="w-full px-4 py-2 border rounded-lg mb-4">
                <label for="current_purchaseprice" class="mb-2 text-gray-700">Current Purchase Price</label>
                <input name="current_purchaseprice" type="number" value="{{$stock->current_purchaseprice}}"
                    class="w-full px-4 py-2 border rounded-lg mb-4">
            ...
            <div class="flex justify-center mt-4">
                <a href="{{ route('smallbusiness.dashboard') }}" class="text-blue-500 hover:text-blue-700">Small
